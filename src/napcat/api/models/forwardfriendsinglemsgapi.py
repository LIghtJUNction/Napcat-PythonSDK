...
class ForwardFriendSingleMsgAPI(BaseModel):
    ...
    endpoint: str = Field(default="forward_friend_single_msg", description="Endpoint")
    method: str = Field(default="POST", description="HTTP方法")
    Req: type[BaseModel] = Field(default=ForwardFriendSingleMsgReq, description="请求模型")
    Res: type[BaseModel] = Field(default=ForwardFriendSingleMsgRes, description="响应模型")
    logger: logging.Logger = Field(default_factory=lambda: logging.getLogger(__name__), description="日志记录器")

    def __init__(self) -> None:
        super().__init__()
        self.logger.debug("初始化 ForwardFriendSingleMsgAPI")

    def __call__(self, req: ForwardFriendSingleMsgReq) -> ForwardFriendSingleMsgRes:
        """
        ...
        """
        # 调试日志
        self.logger.debug(f"调用 ForwardFriendSingleMsgAPI API")
