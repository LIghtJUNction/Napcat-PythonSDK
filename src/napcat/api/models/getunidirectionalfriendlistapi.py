# -*- coding: utf-8 -*-
# region METADATA
"""
@tags: {{tags}}
@homepage: https://napcat.apifox.cn/266151878e0
@llms.txt: https://napcat.apifox.cn/266151878e0.md
@last_update: 2025-04-25 23:00:50

@description: 

summary:获取单向好友列表

"""
__author__ = "LIghtJUNction"
__version__ = "4.7.17"
__endpoint__ = "get_unidirectional_friend_list"
__id__ = "266151878e0"
__method__ = "POST"

# region METADATA/


# region code
from pydantic import BaseModel, Field

import logging

logger = logging.getLogger(__name__)

# region component_models

# region component_models/

# region req
class GetUnidirectionalFriendListReq(BaseModel):
    """获取单向好友列表"""

    model_config = {
        "extra": "allow",
    }
# region req/


# region res
class GetUnidirectionalFriendListRes(BaseModel):
    """获取单向好友列表"""
    status: str = Field(default="ok", description="status字段")
    retcode: float = Field(default=0, description="retcode字段")
    data: list[dict[str, any]] = Field(default_factory=list, description="data字段")
    message: str = Field(default="", description="message字段")
    wording: str = Field(default="", description="wording字段")
    echo: str | None = Field(default=None, description="echo字段")

    model_config = {
        "extra": "allow",
    }

# region res/

# region api
class GetUnidirectionalFriendListAPI(BaseModel):
    """get_unidirectional_friend_list接口数据模型"""
    endpoint: str = Field(default="get_unidirectional_friend_list", description="endpoint")
    method: str = Field(default="POST", description="method")
    Req: type[BaseModel] = Field(default=GetUnidirectionalFriendListReq, description="Req")
    Res: type[BaseModel] = Field(default=GetUnidirectionalFriendListRes, description="Res")
    logger = logger

    def __init__(self) -> None:
        super().__init__()
        self.logger.debug("初始化 GetUnidirectionalFriendListAPI")

    def __call__(self, req: GetUnidirectionalFriendListReq) -> GetUnidirectionalFriendListRes:
        """
        调用API的方法（仅作为接口定义，不包含实际请求逻辑）
        
        Args:
            req: 请求参数对象
            
        Returns:
            响应对象
        """
        # 调试日志
        self.logger.debug(f"调用 GetUnidirectionalFriendListAPI API")
        self.logger.debug(f"请求参数: {req.model_dump_json()}")
        
        # 注意：这里不应该包含实际的请求逻辑
        # 这个方法只是定义了接口，实际的请求应由适配器或客户端实现
        raise NotImplementedError("此方法需要由实际的API客户端实现")

# region api/
# region code/