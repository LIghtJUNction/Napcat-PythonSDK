...
class SetModelShowReq(BaseModel):
    ...
    model_show: str = Field(..., description="展示模型名称")
...
class SetModelShowAPI(BaseModel):
    ...
    method: str = "POST"
    Req: type[SetModelShowReq] = SetModelShowReq
    Res: type[SetModelShowRes] = SetModelShowRes
    logger = logging.getLogger(__name__)

    def __init__(self) -> None:
        super().__init__()
        self.logger.debug("初始化 SetModelShowAPI")

    def __call__(self, req: SetModelShowReq) -> SetModelShowRes:
        """
        ...
        """
        # 调试日志
        self.logger.debug(f"调用 SetModelShowAPI API")
