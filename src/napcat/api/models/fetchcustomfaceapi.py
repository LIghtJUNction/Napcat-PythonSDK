...
class Result(BaseModel):
    ...
    data: list[str] = Field(description="data字段")
...
class FetchCustomFaceReq(BaseModel):
    """获取收藏表情"""
    count: float | None = Field(default=None, description="数量")
...
class FetchCustomFaceAPI(BaseModel):
    ...
    def __init__(self) -> None:
        super().__init__()
        self.logger.debug("初始化 FetchCustomFaceAPI")

    def __call__(self, req: FetchCustomFaceReq) -> FetchCustomFaceRes:
        """
        ...
        """
        # 调试日志
        self.logger.debug(f"调用 FetchCustomFaceAPI API")
