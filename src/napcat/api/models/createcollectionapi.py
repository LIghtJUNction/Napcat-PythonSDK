...
class CreateCollectionRes(BaseModel):
    ...
    status: str = Field(default="ok", description="status字段")
    retcode: float = Field(default=0.0, description="retcode字段")
    data: ResponseData = Field(default_factory=lambda: CreateCollectionRes.ResponseData, description="data字段")
    message: str = Field(default="", description="message字段")
    wording: str = Field(default="", description="wording字段")
    echo: str | None = Field(default=None, description="echo字段")
...
class CreateCollectionAPI(BaseModel):
    ...
    def __init__(self) -> None:
        super().__init__()
        self.logger.debug("初始化 CreateCollectionAPI")

    def __call__(self, req: CreateCollectionReq) -> CreateCollectionRes:
        """
        ...
        """
        # 调试日志
        self.logger.debug(f"调用 CreateCollectionAPI API")
