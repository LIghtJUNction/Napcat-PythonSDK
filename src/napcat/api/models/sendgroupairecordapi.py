...
class SendGroupAiRecordRes(BaseModel):
    ...
    class ResponseData(BaseModel):
        """响应数据类型"""
        message_id: str = Field(description="message_id字段")
...
class SendGroupAiRecordAPI(BaseModel):
    ...
    def __init__(self) -> None:
        super().__init__()
        self.logger.debug("初始化 SendGroupAiRecordAPI")

    def __call__(self, req: SendGroupAiRecordReq) -> SendGroupAiRecordRes:
        """
        ...
        """
        # 调试日志
        self.logger.debug(f"调用 SendGroupAiRecordAPI API")
