...
class ForwardGroupSingleMsgAPI(BaseModel):
    ...
    endpoint: str = Field(default="forward_group_single_msg")
    method: str = Field(default="POST")
    Req: type[BaseModel] = Field(default=ForwardGroupSingleMsgReq)
    Res: type[BaseModel] = Field(default=ForwardGroupSingleMsgRes)
    logger: logging.Logger = Field(default_factory=lambda: logging.getLogger(__name__))

    def __init__(self) -> None:
        super().__init__()
        self.logger.debug("初始化 ForwardGroupSingleMsgAPI")

    def __call__(self, req: ForwardGroupSingleMsgReq) -> Result:
        """
        ...
        """
        # 调试日志
        self.logger.debug(f"调用 ForwardGroupSingleMsgAPI API")
