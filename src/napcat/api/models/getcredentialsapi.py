...
class GetCredentialsReq(BaseModel):
    """获取 QQ 相关接口凭证"""
    domain: str = Field(..., description="domain字段")
...
class GetCredentialsAPI(BaseModel):
    ...
    endpoint: str = Field(default="get_credentials", description="endpoint")
    method: str = Field(default="POST", description="method")
    Req: type[BaseModel] = Field(default=GetCredentialsReq, description="Req")
    Res: type[BaseModel] = Field(default=GetCredentialsRes, description="Res")
    logger: logging.Logger = Field(default_factory=lambda: logging.getLogger(__name__), description="logger")

    def __init__(self) -> None:
        super().__init__()
        self.logger.debug("初始化 GetCredentialsAPI")

    def __call__(self, req: GetCredentialsReq) -> GetCredentialsRes:
        """
        ...
        """
        # 调试日志
        self.logger.debug(f"调用 GetCredentialsAPI API")
