...
class GetVersionInfoRes(BaseModel):
    ...
    class ResponseData(BaseModel):
        """响应数据类型"""
        app_name: str = Field(..., description="app_name字段")
        protocol_version: str = Field(..., description="protocol_version字段")
        app_version: str = Field(..., description="app_version字段")
...
class GetVersionInfoAPI(BaseModel):
    ...
    def __init__(self) -> None:
        super().__init__()
        self.logger.debug("初始化 GetVersionInfoAPI")

    def __call__(self, req: GetVersionInfoReq) -> GetVersionInfoRes:
        """
        ...
        """
        # 调试日志
        self.logger.debug(f"调用 GetVersionInfoAPI API")
