...
class GetCsrfTokenAPI(BaseModel):
    ...
    endpoint: str = Field(default="get_csrf_token", description="接口名称")
    method: str = Field(default="POST", description="请求方法")
    Req: type[BaseModel] = Field(default=GetCsrfTokenReq, description="请求模型")
    Res: type[BaseModel] = Field(default=GetCsrfTokenRes, description="响应模型")
    logger = logging.getLogger(__name__)

    def __init__(self) -> None:
        super().__init__()
        self.logger.debug("初始化 GetCsrfTokenAPI")

    def __call__(self, req: GetCsrfTokenReq) -> GetCsrfTokenRes:
        """
        ...
        """
        # 调试日志
        self.logger.debug(f"调用 GetCsrfTokenAPI API")
