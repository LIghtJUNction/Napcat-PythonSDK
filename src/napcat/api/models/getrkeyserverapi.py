...
class GetRkeyServerRes(BaseModel):
    ...
    class ResponseData(BaseModel):
        """响应数据类型"""
        private_rkey: str = Field(description="private_rkey字段")
        group_rkey: str = Field(description="group_rkey字段")
        expired_time: float = Field(description="expired_time字段")
...
class GetRkeyServerAPI(BaseModel):
    ...
    def __init__(self) -> None:
        super().__init__()
        self.logger.debug("初始化 GetRkeyServerAPI")

    def __call__(self, req: GetRkeyServerReq) -> GetRkeyServerRes:
        """
        ...
        """
        # 调试日志
        self.logger.debug(f"调用 GetRkeyServerAPI API")
