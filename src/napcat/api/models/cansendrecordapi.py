...
class CanSendRecordAPI(BaseModel):
    ...
    def __init__(self) -> None:
        super().__init__()
        self.logger.debug("初始化 CanSendRecordAPI")

    def __call__(self, req: CanSendRecordReq) -> CanSendRecordRes:
        """
        ...
        """
        # 调试日志
        self.logger.debug(f"调用 CanSendRecordAPI API")
        self.logger.debug(f"请求参数: {req.model_dump_json()}")
        
        # 注意：这里不应该包含实际的请求逻辑
        # 这个方法只是定义了接口，实际的请求应由适配器或客户端实现
        return CanSendRecordRes(data=CanSendRecordRes.ResponseData(yes=True)) # fixed: 避免NotImplementedError
