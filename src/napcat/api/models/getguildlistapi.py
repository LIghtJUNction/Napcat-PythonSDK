...
class GetGuildListRes(BaseModel):
    ...
    data: ResponseData = Field(default_factory=lambda: GetGuildListRes.ResponseData(), description="响应数据")
...
class GetGuildListAPI(BaseModel):
    ...
    endpoint: str = Field(default="get_guild_list", description="接口地址")
    method: str = Field(default="POST", description="请求方法")
    Req: type[BaseModel] = Field(default=GetGuildListReq, description="请求模型")
    Res: type[BaseModel] = Field(default=GetGuildListRes, description="响应模型")
    logger = logger
