...
class GetGuildServiceProfileRes(BaseModel):
    """get_guild_service_profile 响应数据"""
    ...
    data: ResponseData = Field(default_factory=ResponseData, description="响应数据")
...
class GetGuildServiceProfileAPI(BaseModel):
    ...
    endpoint: str = Field(default="get_guild_service_profile", description="API端点")
    method: str = Field(default="POST", description="HTTP方法")
    Req: type[BaseModel] = Field(default=GetGuildServiceProfileReq, description="请求模型")
    Res: type[BaseModel] = Field(default=GetGuildServiceProfileRes, description="响应模型")
