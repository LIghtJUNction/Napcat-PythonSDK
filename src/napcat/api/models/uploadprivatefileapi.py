...
class Result(BaseModel):
    ...
    data: dict[str, any] | None = Field(None, description="data字段")
...
class UploadPrivateFileReq(BaseModel):
    ...
    file: str = Field(..., description="文件内容")
...
class UploadPrivateFileRes(BaseModel):
    ...
    data: ResponseData | None = Field(default_factory=lambda: UploadPrivateFileRes.ResponseData(), description="data字段")
...
class UploadPrivateFileAPI(BaseModel):
    ...
    endpoint: str = Field(default="upload_private_file", description="Endpoint")
    method: str = Field(default="POST", description="HTTP方法")
    Req: type[BaseModel] = Field(default=UploadPrivateFileReq, description="请求模型")
    Res: type[BaseModel] = Field(default=UploadPrivateFileRes, description="响应模型")
    logger: logging.Logger = Field(default_factory=lambda: logging.getLogger(__name__), description="日志记录器")

    def __init__(self) -> None:
        super().__init__()
        self.logger.debug("初始化 UploadPrivateFileAPI")

    def __call__(self, req: UploadPrivateFileReq) -> Result:
        """
        ...
        """
        # 调试日志
        self.logger.debug(f"调用 UploadPrivateFileAPI API")
