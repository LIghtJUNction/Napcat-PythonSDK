...
class SendLikeReq(BaseModel):
    """点赞请求参数"""
    user_id: str | float = Field(description="用户ID")
...
class SendLikeAPI(BaseModel):
    """send_like接口数据模型"""
    endpoint: str = Field(default="send_like", description="API端点")
    method: str = Field(default="POST", description="HTTP方法")
    Req: type[BaseModel] = Field(default=SendLikeReq, description="请求模型")
    Res: type[BaseModel] = Field(default=SendLikeRes, description="响应模型")
    logger: logging.Logger = Field(default_factory=lambda: logging.getLogger(__name__), description="日志记录器")

    def __init__(self) -> None:
        super().__init__()
        self.logger.debug("初始化 SendLikeAPI")

    def __call__(self, req: SendLikeReq) -> SendLikeRes:
        """
        ...
        """
        # 调试日志
        self.logger.debug(f"调用 SendLikeAPI API")
