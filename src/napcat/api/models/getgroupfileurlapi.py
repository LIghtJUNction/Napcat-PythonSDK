...
class ResultData(BaseModel):
    url: str = Field(description="url字段")
...
class GetGroupFileUrlAPI(BaseModel):
    ...
    endpoint: str = Field(default="get_group_file_url", description="接口名称")
    method: str = Field(default="POST", description="请求方法")
    Req: type[BaseModel] = Field(default=GetGroupFileUrlReq, description="请求模型")
    Res: type[BaseModel] = Field(default=GetGroupFileUrlRes, description="响应模型")
    logger: logging.Logger = Field(default_factory=lambda: logging.getLogger(__name__), description="日志记录器")

    def __init__(self) -> None:
        super().__init__()
        self.logger.debug("初始化 GetGroupFileUrlAPI")

    def __call__(self, req: GetGroupFileUrlReq) -> GetGroupFileUrlRes:
        """
        ...
        """
        # 调试日志
        self.logger.debug(f"调用 GetGroupFileUrlAPI API")
