...
class NcGetRkeyRes(BaseModel):
    ...
    status: str = Field(default="ok", description="status字段")
    retcode: float = Field(default=0.0, description="retcode字段")
    data: list[RkeyData] = Field(default_factory=list, description="data字段")
    message: str = Field(default="", description="message字段")
    wording: str = Field(default="", description="wording字段")
    echo: str | None = Field(default=None, description="echo字段")
...
class NcGetRkeyAPI(BaseModel):
    """nc_get_rkey接口数据模型"""
    endpoint: str = Field(default="nc_get_rkey", description="Endpoint")
    method: str = Field(default="POST", description="Method")
    Req: type[BaseModel] = Field(default=NcGetRkeyReq, description="Request Model")
    Res: type[BaseModel] = Field(default=NcGetRkeyRes, description="Response Model")
    logger = logging.getLogger(__name__)
