...
class SetGroupCardRes(BaseModel):
    ...
    data: ResponseData | None = Field(default_factory=lambda: SetGroupCardRes.ResponseData(), description="data字段")
...
class SetGroupCardAPI(BaseModel):
    ...
    endpoint: str = Field(default="set_group_card", description="API端点")
    ...
    def __init__(self) -> None:
        super().__init__()
        self.logger.debug("初始化 SetGroupCardAPI")

    def __call__(self, req: SetGroupCardReq) -> Result:
        """
        ...
        """
        # 调试日志
        self.logger.debug(f"调用 SetGroupCardAPI API")
