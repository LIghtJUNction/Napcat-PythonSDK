...
def save_to_model_file(model_name: str, content: str) -> None:
    ...
    try:
        ...
        # 更新模型包的 __init__.py
        update_models_init(models_dir, clean_model_name)
    except Exception as e:
        logger.error(f"保存模型文件失败: {e}")
        raise


def update_models_init(models_dir: Path, model_name: str) -> None:
    """
    更新或创建模型包的 __init__.py 文件，自动导出 API 类
    ...
    # 导出名称转换：获取正确的API类名（从文件读取实际类名）
    class_file = models_dir / f"{model_name}.py"
    api_class_name = ""
    
    logger.debug(f"获取文件 {class_file} 中的实际API类名")
    try:
        if class_file.exists():
            with open(class_file, "r", encoding="utf-8") as f:
                content = f.read()
                # 寻找类名定义 class XXX(BaseModel):
                class_pattern = r"class\s+([A-Za-z][A-Za-z0-9_]*API)\s*\("
                class_match = re.search(class_pattern, content)
                if class_match:
                    api_class_name = class_match.group(1)
                    logger.debug(f"从文件中提取到API类名: {api_class_name}")
                else:
                    # 如果未找到API类，使用传统命名方案作为后备
                    logger.warning(f"未能从文件中提取API类名，使用默认命名规则")
                    api_class_name = ''.join(word.capitalize() for word in model_name.split('_') if word) + 'API'
        else:
            logger.warning(f"类文件 {class_file} 不存在，使用默认API类名")
            api_class_name = ''.join(word.capitalize() for word in model_name.split('_') if word) + 'API'
    except Exception as e:
        logger.error(f"读取类文件失败: {e}")
        api_class_name = ''.join(word.capitalize() for word in model_name.split('_') if word) + 'API'
    
    logger.debug(f"最终使用的API类名: {api_class_name}")
    
    # 如果文件不存在，则创建基础内容
    if not init_file.exists():
        logger.debug(f"创建模型包初始化文件: {init_file}")
        with open(init_file, "w", encoding="utf-8") as f:
            f.write('"""Napcat API 模型包\n\n自动生成的API模型类\n"""\n\n')
            f.write('__all__ = [  # 导出所有API类\n')
            f.write(f'    "{api_class_name}",\n')
            f.write(']\n\n')
            f.write(f'from .{model_name} import {api_class_name}\n')
        logger.info(f"已创建 __init__.py 并导出 {api_class_name}")
        return
    ...
    if all_match:
        ...
        # 检查API类是否已存在
        if f'"{api_class_name}"' not in all_content:
            # 将新API类添加到列表末尾
            new_all_content = all_content.rstrip() + f'    "{api_class_name}",\n'
            content = content.replace(all_match.group(1), new_all_content)
            logger.debug(f"已将 {api_class_name} 添加到 __all__ 列表")
    else:
        # 如果没有找到 __all__，添加一个
        logger.debug("未找到 __all__ 列表，将创建一个")
        content = '"""Napcat API 模型包\n\n自动生成的API模型类\n"""\n\n'
        content += f'__all__ = [  # 导出所有API类\n    "{api_class_name}",\n]\n\n'
    
    # 添加导入语句
    import_line = f"from .{model_name} import {api_class_name}"
    if import_line not in content:
        content += f"\n{import_line}\n"
        logger.debug(f"已添加导入语句: {import_line}")
    
    # 写回文件
    with open(init_file, "w", encoding="utf-8") as f:
        f.write(content)
    
    logger.info(f"已更新 __init__.py 并导出 {api_class_name}")
